...
Análise detalhada das reservas totais ao longo do tempo especificamente para China, Japão e Estados Unidos, destacando diferenças estratégicas e tendências particulares observadas nessas grandes economias ao longo dos anos.
...
selected_countries <- c("United States", "China", "Japan")  # ajuste conforme necessário

time_series_data <- reserveall_top10 %>%
  filter(country %in% selected_countries)

ggplot(time_series_data, aes(x = year, y = FI.RES.TOTL.CD, color = country)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(title = "Séries Temporais de Reservas Totais",
       subtitle = "Ano: {frame_along}",
       x = "Ano",
       y = "Reservas Totais (US$)",
       color = "País") +
